...
namespace Lampac.Controllers.Durex
{
    public class LumexController : BaseOnlineController
    {
        [HttpGet]
        [Route("vcdn")]
        async public Task<ActionResult> Index(long content_id, string content_type, string imdb_id, long kinopoisk_id, string title, string original_title, string t, int clarification, int s = -1, int serial = -1, bool rjson = false, bool checksearch = false)
        {
            if (content_id == 0)
            {
                var search = await InvokeCache($"durexmod:search:{imdb_id}:{kinopoisk_id}:{title ?? original_title}:{clarification}", TimeSpan.FromHours(1), () => Search(imdb_id, kinopoisk_id, title, original_title, serial, clarification));
                if (search.content_type == null && search.similar.data.Count == 0)
                    return OnError();

                if (search.similar.data.Count != 0)
            ...
            var rch = new RchClient(HttpContext, host, ModInit.vsdn, requestInfo, keepalive: serial == 0 ? null : -1);
            ...
            if (player.content_type is "movie" or "anime")
            {
                ...
                {
                    string link = accsArgs($"{host}/vcdn/video?content_id={content_id}&content_type={content_type}&playlist={HttpUtility.UrlEncode(media.playlist)}&max_quality={media.max_quality}&translation_id={media.translation_id}");
            ...
            else
            {
                ...
                if (s == -1)
                {
                    ...
                    {
                        string link = $"{host}/vcdn?content_id={content_id}&content_type={content_type}&title={enc_title}&original_title={enc_original_title}&s={media.season_id}";
                    ...
                    foreach (var media in player.media)
                    {
                        ...
                        foreach (var episode in media.episodes)
                        {
                            foreach (var voice in episode.media)
                            {
                                ...
                                vtpl.Append(voice.translation_name, t == voice.translation_id.ToString(), $"{host}/vcdn?content_id={content_id}&content_type={content_type}&title={enc_title}&original_title={enc_original_title}&s={s}&t={voice.translation_id}");
                            }
                        }
                    }
                    #endregion
                    ...
                    foreach (var media in player.media)
                    {
                        ...
                        foreach (var episode in media.episodes)
                        {
                            foreach (var voice in episode.media)
                            {
                                ...
                                string link = accsArgs($"{host}/vcdn/video?content_id={content_id}&content_type={content_type}&playlist={HttpUtility.UrlEncode(voice.playlist)}&max_quality={voice.max_quality}&s={s}&e={episode.episode_id}&translation_id={voice.translation_id}&serial=true");
                                string streamlink = link.Replace("/video", "/video.m3u8") + "&play=true";
        ...
        {
            var rch = new RchClient(HttpContext, host, ModInit.vsdn, requestInfo, keepalive: serial ? - 1 : null);
            if (rch.IsNotConnected())
                return ContentTo(rch.connectionMsg);
            ...
            string memkey = $"durexmod/video:{playlist}:{requestInfo.IP}";
            if (!hybridCache.TryGetValue(memkey, out string hls))
            {
                ...
                var result = await Http.Post<JObject>(ModInit.vsdn.apihost + playlist, "{}", headers: headers);
            ...
            var player = await getPlayer(content_id, content_type, accessToken);
            var vast = new VastConf() { url = player?.tag_url, msg = ModInit.vast_msg };
                                    ...
                                    foreach (var voice in episode.media.Where(i => i.translation_id == translation_id))
                                    {
                                        if (voice.tracks != null)
                                        {
                                            foreach (var t in voice.tracks)
                                                subtitles.Append(t.label ?? t.srlang, $"{ModInit.vsdn.scheme}:{t.src}");
            ...
            if (max_quality > 0 && !ModInit.vsdn.hls)
            {
        ...
        {
            #region refreshToken
            string memKey = "durexmod:refreshToken";
            if (!memoryCache.TryGetValue(memKey, out string refreshToken))
            {
                var data = new System.Net.Http.StringContent($"{{\"username\":\"{ModInit.username}\",\"password\":\"{ModInit.password}\"}}", Encoding.UTF8, "application/json");
                var job = await Http.Post<JObject>($"{ModInit.vsdn.apihost}/login", data, useDefaultHeaders: false);
                ...
                memoryCache.Set(memKey, refreshToken, DateTime.Now.AddDays(2));
            }
            #endregion

            memKey = $"durexmod:accessToken:{requestInfo.IP}";
            if (!hybridCache.TryGetValue(memKey, out string accessToken))
            {
                ...
                var job = await Http.Post<JObject>($"{ModInit.vsdn.apihost}/refresh", data, timeoutSeconds: 5, useDefaultHeaders: false, headers: HeadersModel.Init(("x-forwarded-for", requestInfo.IP)));
        ...
        async ValueTask<EmbedModel> getPlayer(long content_id, string content_type, string accessToken)
        {
            ...
            return await InvokeCache($"durexmod:{content_id}:{content_type}:{accessToken}:{requestInfo.IP}", TimeSpan.FromMinutes(5), async () =>
            {
                ...
                string json = await Http.Get($"{ModInit.vsdn.apihost}/stream?clientId={ModInit.vsdn.clientId}&contentType={content_type}&contentId={content_id}&domain={ModInit.domain}", useDefaultHeaders: false, timeoutSeconds: 8, headers: headers);
        ...
        #region Search
        async ValueTask<(long content_id, string content_type, SimilarTpl similar)> Search(string imdb_id, long kinopoisk_id, string title, string original_title, int serial, int clarification)
        {
            async ValueTask<JToken> searchId(string imdb_id, long kinopoisk_id)
            {
                ...
                var job = await Http.Get<JObject>($"{ModInit.vsdn.iframehost}/api/short?api_token={ModInit.vsdn.token}" + arg, timeoutSeconds: 8);
            ...
            if (movie != null)
            {
                return (movie.Value<long>("id"), movie.Value<string>("content_type"), new SimilarTpl());
            }
            else
            {
                ...
                string uri = $"{ModInit.vsdn.iframehost}/api/short?api_token={ModInit.vsdn.token}&title={HttpUtility.UrlEncode(clarification == 1 ? title : (original_title ?? title))}";
                string json = await Http.Get(uri, timeoutSeconds: 8);
                if (json == null)
                    return default;
                ...
                foreach (var item in root.data)
                {
                    ...
                    stpl.Append(name, year, details, $"{host}/vcdn?title={enc_title}&original_title={enc_original_title}&content_id={item.id}&content_type={item.content_type}");
